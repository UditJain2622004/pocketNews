...
def generate_story_visuals(
    ...
    paths = [_relative(reference_path)]
    previous_path = reference_path
    for beat in beats:
        ...
        if not image_path.is_file():
            image_path.write_bytes(
                generate_story_image(
                    ...
                    reference_images=[reference_path] if previous_path == reference_path else [reference_path, previous_path],
                )
            )
            generated += 1
        else:
    ...
    return {"paths": paths, "generated": generated, "reused": reused}
...
def _story_context(story: dict[str, Any], visual_bible: dict[str, Any]) -> str:
    ...
    beat_sequence = " ".join(
        f"{beat.get('id')}: {beat.get('visual', {}).get('scenePrompt') or beat.get('visual', {}).get('imagePrompt')}"
    ...
    return (
        ...
        f"Story arc: {visual_bible.get('storyArc')}. Characters: {cast_details}. Full shot sequence: {beat_sequence}"
    )
