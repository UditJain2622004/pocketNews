...
CASTS = {
    ...
    "two-person-banter": [
        {"id": "maya", "role": "explainer", "voiceProfile": "warm, nimble, conversational", "language": "en-IN"},
        {"id": "leo", "role": "skeptical friend", "voiceProfile": "dry humor, curious", "language": "en-IN"},
    ],
...
def build_episode(
    articles: list[NewsArticle], interests: list[str], cadence: str, language: str, story_count: int
) -> dict[str, object]:
...
def build_story(
    ...
    selected_format = story_format or _story_format_for_index(index)
    summary = _story_summary(article.best_available_text, article.title)
    beats = _build_beats(article, summary, category, details, selected_format)
    source = {"name": article.source_name, "url": article.url, "publishedAt": article.published_at}
    return {
        "storyId": article.id,
        "title": article.title,
        "skipLabel": f"{details['label']}: {article.title}",
        "category": category,
        "topics": article.categories,
        "durationSeconds": sum(beat["visual"]["durationSeconds"] for beat in beats),
        "sources": [source],
        "format": selected_format,
        "cast": [{**member, "language": language} for member in CASTS[selected_format]],
        "entry": {
            "direct": f"Quick story: {article.title}.",
            "afterRelated": f"Staying on {details['label'].lower()}, {article.title}.",
            "afterUnrelated": f"Different kind of plot twist: {article.title}.",
        },
        "beats": beats,
        "exit": f"Bottom line: {article.title}",
    }
...
def _build_beats(
    ...
    prompt_base = (
        f"Vertical 9:16 cinematic editorial-comedy visual about {details['label'].lower()} news. "
        f"Story context: {article.title}. Dramatic film lighting, premium contemporary setting, "
        "clear upper-third negative space for a caption, no text, logos, watermarks, fake screenshots, "
        "or realistic portrayal of a public figure doing an unverified action."
    )
...
def _visual(kind: str, duration: int, caption: str, prompt: str, details: dict[str, str]) -> dict[str, object]:
    return {
        ...
        "imagePrompt": prompt,
        "motion": "slow push-in" if kind != "character-reaction" else "gentle pan",
        "tone": details["color"],
    }
...
def _impact_lines(story_format: str, category: str) -> list[dict[str, str]]:
    ...
    if story_format == "two-person-banter":
        return [
            {"speaker": "leo", "text": "And why should I care before my next snack break?"},
            {"speaker": "maya", "text": f"Because this is the kind of {label} move that can shape what happens next, not just today's headline."},
        ]
...
def _story_format_for_index(index: int) -> str:
    return supported_story_formats()[index % len(CASTS)]
